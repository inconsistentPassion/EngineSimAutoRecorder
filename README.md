...
## Python version (`python_version/engine_auto_recorder.py`)
...
| Engine Simulator (running) | The window must be visible / focused |
| A loopback audio device | Windows: enable *Stereo Mix*; Linux: PulseAudio monitor source |

### Install dependencies

```bash
pip install paddlepaddle paddleocr mss sounddevice numpy scipy pynput
```

> **GPU acceleration (optional):** Replace `paddlepaddle` with
> `paddlepaddle-gpu` if you have a CUDA-capable GPU — OCR will run
> significantly faster.

### Tuning the OCR coordinates (Python version)

Run the built-in screenshot helper:

```bash
cd python_version
python engine_auto_recorder.py --find-region
```

This saves `screen_capture.png` to the current directory.  Open it in any
image editor (Paint, GIMP, etc.), hover over the RPM digits, and note the
pixel coordinates.  Then edit the `OCR_REGION` constant at the top of the
script:

```python
OCR_REGION: dict = {"left": 860, "top": 45, "width": 160, "height": 40}
#                            ^^^         ^^^           ^^^            ^^^
#                            X           Y             W              H
```
...
# Use default target RPMs (800, 1500, 3000, 4500, 6000, 7500)
python engine_auto_recorder.py

# Custom RPM list
python engine_auto_recorder.py --rpms 1000 2000 4000 6000

# Custom output directory
python engine_auto_recorder.py --output my_recordings/
```
